# Engram — project orientation

Cures the collector's fallacy: turns a static second brain into durable memory via AI-orchestrated,
gamified spaced repetition.

- **Design source of truth:** `docs/V1-spec.md` — read it before writing code. Locked invariants, quiz-surface laws, data model, pricing, and the build sequence all live there.
- **Live status & current task:** `progress.md` (not here — this file holds durable orientation + rules only).
- **Current task:** ENG-1, the de-risking spike — see `backend/README.md`.
...
Persistent lessons live in `learnings.md` — read it at the start of every task and follow every rule there.
